...
namespace PBR {
    ...
    // Helper function to convert direction to equirectangular MS
    float2 directionToEquirectangularMS(float3 direction) {
    ...
    fragment float4 fragment_main(
        ...
        if (!is_null_texture(environmentTexture)) {
            ...
            // Convert reflection vector to equirectangular MS coordinates
            float2 envMS = directionToEquirectangularMS(R);

            // Sample environment map - use roughness for mip level
            float mipLevel = roughness * 8.0; // Assume 8 mip levels
            float3 envColor = environmentTexture.sample(envSampler, envMS, level(mipLevel)).rgb;
            ...
            float2 irradianceMS = directionToEquirectangularMS(N);
            float3 irradiance = environmentTexture.sample(envSampler, irradianceMS, level(8.0)).rgb * 0.3;
            ...
            if (material.clearcoat > 0.001) {
                ...
                float3 clearcoatEnv = environmentTexture.sample(envSampler, envMS, level(clearcoatMipLevel)).rgb;
