...
namespace Panorama {
    ...
    struct SphereVertexOut {
        ...
        float3 modelSpacePosition; // Pass model-space position for MS calculation in fragment
    };
    ...
    [[fragment]] float4 fragment_main(
        ...
        // Calculate MS using the camera function with location and rotation support
        ...
        // Check debug mode
        if (uniforms && uniforms->showMS) {
            // Show MS coordinates as colors (R=U, G=V, B=0)
    ...
    minimap_fragment(MinimapVertexOut in [[stage_in]], texture2d<float> panoramaTexture [[texture(0)]]) {
        ...
        // Convert MS to centered coordinates
        ...
        // Convert spherical coordinates to equirectangular MS coordinates
