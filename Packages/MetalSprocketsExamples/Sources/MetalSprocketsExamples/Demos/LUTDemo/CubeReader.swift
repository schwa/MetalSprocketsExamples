...
struct CubeReader {
    ...
    init(url: URL) throws {
        ...
        for line in lines {
            ...
            if title == nil, let match = try titleRegex.firstMatch(in: String(trimmedLine)) {
                ...
                guard components.count == 3 else {
                    throw MetalSprocketsError.validationError("Invalid LUT entry: expected 3 components, got \(components.count)")
                }
        ...
        guard is3D, let count else {
            throw MetalSprocketsError.configurationError("Missing or invalid LUT_3D_SIZE declaration in .cube file")
        }

        guard values.count == count * count * count else {
            throw MetalSprocketsError.validationError("LUT data size mismatch: expected \(count * count * count) entries, got \(values.count)")
        }
