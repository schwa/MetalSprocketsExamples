...
struct CheckerboardKernel: Element {
    ...
    init(outputTexture: MTLTexture, checkerSize: SIMD2<Float>, foregroundColor: SIMD4<Float>) throws {
        kernel = try ShaderLibrary(bundle: .metal-sprocketsExampleShaders(), namespace: "Checkerboard").CheckerboardKernel_float4
...
struct CheckerboardKernel_ushort: Element {
    ...
    init(outputTexture: MTLTexture, checkerSize: SIMD2<Float>, foregroundColor: UInt16) throws {
        kernel = try ShaderLibrary(bundle: .metal-sprocketsExampleShaders(), namespace: "Checkerboard").CheckerboardKernel_ushort
        self.outputTexture = outputTexture
        self.checkerSize = checkerSize
        self.foregroundColor = foregroundColor
    }
